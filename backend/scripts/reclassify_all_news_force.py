...
async def main():
    ...
    async with SessionLocal() as db:
        # SELECT ALL ARTICLES
        result = await db.execute(select(NewsArticleModel).order_by(NewsArticleModel.published_at.desc()))
    ...
    print(f"Found {len(article_dicts)} total articles in DB. Preparing for full re-classification...")
    ...
    for i, row in enumerate(article_dicts):
        ...
        await asyncio.sleep(1) # Reduced sleep to speed up

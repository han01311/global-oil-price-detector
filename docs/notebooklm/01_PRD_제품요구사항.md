# PRD: 글로벌 유가 추정 및 인텔리전스 대시보드 (OilLens)
...
### F5. 공공데이터 통합 (data.go.kr)

- 한국수출입은행 환율 API 연동: KRW/USD 매매기준율 일별 자동 수집 (24시간 주기)
- 한국석유공사(KNOC) 데이터:
  - 국내 원유수입 국가별 통계 (1980~2024, 48개국) → 수입 집중도(HHI) 분석
  - 세계 원유 수출입 물량 매트릭스 (21개 수출국 × 15개 수입국) → 교역 흐름 패턴 분석
- 예측 모델 피처 확장: `krw_usd`, `krw_usd_change_5d` 거시경제 피처 추가
- 펀더멘탈 예측 달러 시그널에 원/달러 환율 보조 정보 주입

### F6. USD/KRW 통화 토글 (Dashboard Currency Toggle)

- 대시보드 유가 예측 테이블에 USD ↔ KRW 전환 토글 배치
- KRW 모드 선택 시 한국수출입은행 매매기준율 기반 원화 환산 표시 (₩ 접두사, 천단위 콤마)
- 환율 배지(`1 USD = ₩X,XXX 한국수출입은행`)로 기준 환율 및 출처 명시
- 환율 로드 실패 시 USD 전용 모드로 폴백 (Graceful Degradation)
...
| **뉴스 API 서버 (NYT/Guardian) 다운** | NYT Article Search API 또는 Guardian Open Platform API 장애로 최신 기사 수집 불가 | - 1차: 최근 24~48시간 동안 적재된 캐시 뉴스 활용<br>- 2차: `market_memory.py`에서 캐시마저 없을 경우, 추정 유가를 정성(뉴스) 보정 없이 정량(Baseline) 수치로만 표기 (안내 문구: "최신 뉴스 데이터 지연") |
...
| **한국수출입은행** (data.go.kr) | KRW/USD 매매기준율 (일별) | REST API (공공데이터 포털 키) | 무료 (1,000회/일) |
| **한국석유공사 KNOC** (data.go.kr) | 원유수입 국가별 통계, 세계 원유 수출입 물량 | 파일 데이터 (CSV, EUC-KR) | 무료 |
...
| **수입 집중도(HHI)** | KNOC 국가별 수입량 → 허핀달-허슈만 지수 산출 (1,000~10,000) |
| **KRW 환산 유가** | USD 유가 × 한국수출입은행 매매기준율 (일별 자동 수집) |
...
[Method A: AI Quant Baseline]
  ...
  피처 확장: krw_usd, krw_usd_change_5d (공공데이터 환율)

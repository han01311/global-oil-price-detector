...
def executor(tmp_project, phase_dir):
    """테스트용 StepExecutor 인스턴스. git/gemini 호출은 별도 mock 필요."""
    with patch.object(ex, "ROOT", tmp_project), \
         patch.object(ex.StepExecutor, "_init_gemini"):
...
# ---------------------------------------------------------------------------
# _parse_and_apply_response
# ---------------------------------------------------------------------------

class TestParseAndApplyResponse:
    def test_parse_valid_json(self, executor):
        ...
        result = executor._parse_and_apply_response(f"```json\n{response}\n```", 2, "ui")
        assert result["status"] == "completed"

    def test_parse_invalid_json(self, executor):
        result = executor._parse_and_apply_response("not json at all", 2, "ui")
